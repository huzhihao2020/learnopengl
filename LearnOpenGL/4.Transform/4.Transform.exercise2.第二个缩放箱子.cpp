...
std::string VertesShaderPath = FolderPath + "/4.Transform-3.3.shader.vs";
std::string FragmentShaderPath = FolderPath + "/4.Transform-3.3.shader.fs";
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ourShader.use();
        trans = glm::rotate(trans, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f)); // 因为glm::rotate的参数是 trans和time，所以每次trans的定义要放在循环里面，否则对trans的影响是累计的，不是匀速转的效果
        unsigned int transformLoc = glGetUniformLocation(ourShader.ID, "trans_mat");
        ...
        trans = glm::mat4(1.0f);
        float scale_f = abs(sin((float)glfwGetTime())/2);
        trans = glm::translate(trans, glm::vec3(-0.5f, 0.5f, 0.0f));
        trans = glm::scale(trans, glm::vec3(scale_f, scale_f, scale_f));
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(trans));
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
